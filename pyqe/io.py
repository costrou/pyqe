...
"""
import re
double_regex = re.compile(r'[-+]?\d+\.\d+(?:[eE][-+]?\d+)?')
...
def read_out_file(output_str):
    ...
    print(len(scf_steps), len(bfgs_steps))
...
def read_charge_density_file(self, inputfile):
    ...
    nrz_regex = (
        b'<z\.{0} type="(\w+)" size="(\d+)" kind="(\d+)">\n'
        b'[\0-\xFF]{{12}}([\0-\xFF]{1})[\0-\xFF]{{24}}'
        b'\n    </z\.{0}>\n'
    )

    match = re.search(info_regex, charge_xml_str).groups()
    nr1, nr2, nr3 = [int(_.decode()) for _ in match]

    data = []
    for nrz in range(nr3):

        _type, size, kind, byte_str = re.search(
            nrz_regex.format(nrz+1, nr1*nr2),
            charge_xml_str)
...
def read_eigenvalue_file(self, inputfile):
    ...
    # Sloppy regex for doubles...
    double_regex = re.compile('[-+]?\d+\.\d+(?:[eE][-+]?\d+)?')
